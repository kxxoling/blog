...
说到 class handler，web.py 就是其中很有名的一个，而 Tornado 的最初的灵感就是来自 web.py，Tornado 和 web.py 在路由方面都是使用路由元组做配置。
...
Flask 在路由方面[支持惰性加载](http://flask.pocoo.org/docs/0.10/patterns/lazyloading/)，提供了 `Flask.add_url_rule()` 函数用于手动注册路由：
