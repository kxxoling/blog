...
| ident    | 根据客户端用户用户名匹配数据库用户名。（远程通过 TCP/IP 连接通信，本地通信决定于系统用户）                       |
| ldap     | 使用 LDAP 服务器认证。                                                                                           |
...
意思是，对于本机登录用户 windrunner 访问 windrunner_db 数据库是不需要安全验证的，对于 10.1.1.0/24 网段的所有用户采用 md5 加密的密码验证方式。

详细可参考 [PostgreSQL wiki](https://wiki.postgresql.org/wiki/9.1第十九章)。其它参考：[Pluggable Authentication Modules，PAM]
