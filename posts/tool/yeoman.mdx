...
module.exports = generators.Base.extend({
  app() {
    this.directory(this.templatePath('src'), './') // 将 templates 目录中的 src 目录复制到新建项目的根目录
...
上面也简单提到了，实现一个 generator 主要就是实现入口文件中的 generator.Base 实例，因此了解其 API 是必须的。API 主页：https://yeoman.github.io/generator/
