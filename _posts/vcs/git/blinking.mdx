...
首先，输入 `git commit` 进入默认编辑器修改 commit 信息，进入 Vim 的插入模式，输入 `Ctrl + v`，放手后再按 `Esc` 键即可得到形如 `^[` 的字符（_实际上并不是_）； 紧接着再输入 `[5m`，之后再输入 commit 信息（这里假设内容为 `COMMIT_MESSAGE`）； 输入后再如同前面输入类似 `^[` 的字符，在输入 `[0m`。
