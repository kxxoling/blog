...
REPL 环境对于学习一门新编程语言的帮助是非常明显的，所以 REPL 工具的好坏非常重要！ Ruby 虽然已经自带了 `irb` ，但鉴于其智能程度实在难有作为。除 `irb` 外，Ruby 也有一些其它的 REPL 工具，但强大程度仅相当于 Python 的 BPython。与 IPython 同源的 SciRuby 是个非常好的选择（IRuby 已废弃，代码库已合并进 SciRuby）。虽然尚未找到能匹敌 ptIPython 的工具，SciRuby + Jupyter Notebook 也足够使用了。
...
（可能在安装 nmatrix 的时候出现编译错误，这时候主要功能基本已经安装完成了，运行 `iruby register --force` 强制注册 Homebrew 安装的 Ruby 即可。）
