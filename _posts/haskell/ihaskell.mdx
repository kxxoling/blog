---
title: IHaskell & notebook
tags: [Haskell, Jupyter]
...
IHaskell 提供了 Ubuntu 和 Mac OS X 下的[安装脚本](https://github.com/gibiansky/IHaskell#install-using-installation-scripts)，其它平台可以手动安装 IHaskell 的依赖和 IHaskell 本身：
...
启动 IHaskell Notebook 前，首先需要在 Jupyter Notebook 中注册 IHaskell Kernel：`ihaskell install`，然后运行 `jupyter notebook` 启动 Jupyter Notebook ，并新建一个 IHaskell kernel 的 Notebook。
